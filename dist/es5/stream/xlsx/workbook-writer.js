...
var WorkbookWriter = module.exports = function (options) {
  ...
  pipeline(this.zip, this.stream); // these bits can be added right now
...
WorkbookWriter.prototype = {
  ...
  _openStream: function _openStream(path) {
    ...
    stream.on('finish', function () {
      console.dir('ssssssssssssssssssss')
      stream.emit('zipped');
      stream.destroy();
    });
    return stream;
    ...
    var commitWorksheet = function commitWorksheet(worksheet) {
      if (!worksheet.committed) {
        return new PromiseLib.Promise(function (resolve) {
          worksheet.stream.once('zipped', function () {
            resolve();
          });
          worksheet.commit();
        });
  ...
  commit: function commit() {
    ...
    return this.promise.then(function () {
      ...
      return PromiseLib.Promise.all([_this.addContentTypes(), _this.addApp(), _this.addCore(), _this.addSharedStrings(), _this.addStyles(), _this.addWorkbookRels()]);
    }).then(function () {
