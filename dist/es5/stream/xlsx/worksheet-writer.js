...
var PictureXform = require('../../xlsx/xform/sheet/picture-xform'); // since prepare and render is functional, we can use singletons
...
var WorksheetWriter = module.exports = function (options) {
  ...
  this._sheetRelsWriter = new SheetRelsWriter(options); // keep a record of dimensions
  ...
  this._workbook = options.workbook; // views
...
WorksheetWriter.prototype = {
  ...
  commit: function commit(cb) {
    ...
    _this._writeRow(this._rows, (err) => {
      this._rows = null;
      ...
      this.stream.end(); // also commit the hyperlinks if any
  ...
  _commitRow: function _commitRow(cRow, cb) {
  ...
  _writeRow: function _writeRow (rows, cb) {
    if (!this.startedData) {
      this._writeColumns();
    ...
    if (!Array.isArray(rows)) {
      ...
      for  (const row of rows) {
        if (!row || (!row.hasValues && !row.height)) continue;
        ...
        arr.push(xform.row.toXml(model));
      }
      this._writeStream(arr, this.stream, cb);
  },

  _writeStream: (arr, stream, cb) => {
    async function asyncForEach(array, callback) {
      for (let index = 0; index < array.length; index++) {
        await callback(array[index], index, array);
      }
    }
    const waitFor = (stream, el) => new Promise(r => {
      if (!stream.write(el)) {
        stream.once('drain', r);
      } else {
        process.nextTick(r);
    ...
    asyncForEach(arr, async (el) => {
      await waitFor(stream, el)
    });
    cb();
  },
  ...
  _writeDimensions: function _writeDimensions() {// for some reason, Excel can't handle dimensions at the bottom of the file
